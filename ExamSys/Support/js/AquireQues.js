    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.success == 1) {
                ...
                $('#probar').css('width', ((info.correctNum + info.wrongNum) / info.total) * 100 + '%');
                $('#total').text("当前总分：" + info.total);
                $('#rightNum').text("正确数：" + info.correctNum);
                $('#wrongNum').text("错误数：" + info.wrongNum);
                $('#allNum').text("题库总题数：" + info.allNum);
                ...
                $('#QuesContent').text("<font size='8px'>再接再厉！！</font>");
                $('#tips').text('请点击退出考试按钮');
            }
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) {
